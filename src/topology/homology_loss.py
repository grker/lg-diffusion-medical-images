...
class PersistentHomologyLoss(torch.nn.Module):
    def __init__(self, config: PersistanceHomologyConfig):
        ...
        self.topo_features = self.check_topofeatures(
            config["topo_features"], self.num_classes
        )
    ...
    def forward(self, x: torch.Tensor):
        assert len(x.shape) == 5, (
            "Tensor should be a 5 dimensional tensor (batch, classes, channels, height, width)"
        )

        assert x.shape[1] != self.num_classes, (
            f"Shape does not match. Tensor has size {x.shape[1]} in dimension 1, but is expected to have {self.num_classes}."
        )
        ...
        for batch_idx in range(x.shape[0]):
            for class_idx in range(x.shape[1] - 1):  # background can be ignored
                cp = gudhi.CubicalComplex(
                    top_dimensional_cells=x[:, class_idx + 1, :, :]
                )
                persistence_list = cp.persistence()

                print(f"Persistence list: {persistence_list}")

                loss += self.loss_per_dimension(
                    [feature[1] for feature in persistence_list if feature[0] == 0],
                    self.topo_features[class_idx + 1][0],
                )  # homology dimension 0
                loss += self.loss_per_dimension(
                    [feature[1] for feature in persistence_list if feature[0] == 1],
                    self.topo_features[class_idx + 1][1],
