...
def generate_preprocessor(config: PreprocessorConfig):
    ...
    for transform_name, kwargs in config.transform_config.items():
        apply_to_mask = (
            kwargs.apply_to_mask if kwargs.apply_to_mask is not None else False
        )
        args = kwargs.args if kwargs.args is not None else {}

        if hasattr(monai.transforms, transform_name):
            preprocessor[transform_name] = getattr(monai.transforms, transform_name)(
                **args
            )
            apply_to_mask[transform_name] = apply_to_mask
        else:
            raise ValueError(
                f"Unknown transform {transform_name}. The preprocessor so far only supports transforms from monai.transforms"
            )
...
def apply_preprocessor(
    preprocessor: dict, apply_to_mask: dict, images: torch.Tensor, masks: torch.Tensor
):
