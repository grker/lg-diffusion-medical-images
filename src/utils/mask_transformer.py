...
class MultiClassSegMaskMapping(BaseMaskMapping):
    ...
    def __init__(
        ...
        if prediction_type == "sample" and switch:
            logger.warning(
                "Switch is set to True but prediction_type is set to sample. Switch will be ignored."
            )
        ...
        if dataset_mapping["background"] is not None and isinstance(
            dataset_mapping["background"], int
        ):
            self.gt_mapping["background"] = (
                0  # background class is always 0 --> ensures that the metrics work as expected
            )
        else:
            raise ValueError(
                f"Background class is not defined in dataset_mapping: {dataset_mapping}."
            )
        ...
        for keys, value in items:
            if keys != "background":
                if not isinstance(value, int):
                    raise TypeError(
                        f"Value of Key {keys} has type {type(value)} but should have been {int}"
                    )
                else:
    ...
    def dataset_to_gt_mask(self, mask: torch.Tensor):
        ...
        for class_name in self.dataset_mapping.keys():
            gt_mask = torch.where(
                mask == self.dataset_mapping[class_name],
                self.gt_mapping[class_name],
                gt_mask,
            )
    ...
    def gt_to_dataset_mask(self, mask: torch.Tensor):
        ...
        for class_name in self.gt_mapping.keys():
            dataset_mask = torch.where(
                mask == self.gt_mapping[class_name],
                self.dataset_mapping[class_name],
                dataset_mask,
            )
    ...
    def gt_to_train_mask(self, mask: torch.Tensor):
        ...
        train_mask = torch.empty(
            mask.shape[0], self.get_num_train_channels(), mask.shape[2], mask.shape[3]
        )

        for class_name, values in self.gt_mapping.items():
            train_mask[:, values, :, :] = torch.where(
                mask == values, fg_value, 1 - fg_value
            ).squeeze(1)
    ...
    def get_segmentation(self, logits: torch.Tensor):
        ...
        if self.ensemble_mode == "mean":
            ...
            segmentations_across_reps = torch.argmax(logits, dim=2, keepdim=True).type(
                torch.int32
            )
...
class BinarySegMaskMapping(BaseMaskMapping):
    ...
    def check_input(self, dictionary):
        if dictionary is not None:
            if dictionary["background"] is None or not isinstance(
                dictionary["background"], int
            ):
                raise ValueError(
                    f"Background class is not defined in dataset_mapping: {dictionary}."
                )
            if dictionary["foreground"] is None or not isinstance(
                dictionary["foreground"], int
            ):
                raise ValueError(
                    f"Foreground class is not defined in dataset_mapping: {dictionary}."
                )
        else:
            raise ValueError("dataset_mapping is not defined.")
        return dictionary

    def set_threshold_func(self, threshold: int):
        if self.train_mapping["foreground"] > self.train_mapping["background"]:
            self.threshold_func = lambda tensor: torch.where(
                tensor > threshold,
                self.train_mapping["foreground"],
                self.train_mapping["background"],
            )
        else:
            self.threshold_func = lambda tensor: torch.where(
                tensor > threshold,
    ...
    def dataset_to_gt_mask(self, mask: torch.Tensor):
        gt_mask = torch.where(
            mask == self.dataset_mapping["background"],
            self.gt_mapping["background"],
            self.gt_mapping["foreground"],
        )
        return gt_mask.type(mask.type())

    def gt_to_dataset_mask(self, mask: torch.Tensor):
        dataset_mask = torch.where(
            mask == self.gt_mapping["background"],
            self.dataset_mapping["background"],
            self.dataset_mapping["foreground"],
        )
        return dataset_mask.type(mask.type())
    ...
    def get_segmentation(self, logits: torch.Tensor):
        ...
        if self.ensemble_mode == "mean":
            ...
            raise ValueError(
                f"Ensemble mode {self.ensemble_mode} has not been implemented."
            )
...
def generate_mask_mapping(config: MaskTransformerConfig) -> BaseMaskMapping:
    if config.mask_type == "binary":
        return BinarySegMaskMapping(
            config.dataset_mapping, config.train_mapping, config.threshold
        )
    elif config.mask_type == "multi_class":
        return MultiClassSegMaskMapping(
            config.dataset_mapping, config.train_switch, config.prediction_type
        )
    else:
