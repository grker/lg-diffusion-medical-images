...
python src/main.py project_name=dmiise validation_period=10 trainer.max_epochs=100 dataloader.batch_size=128 dataloader.val_batch_size=128 diffusion=sample_diffusion dataset=mnist_labeled diffusion.repetitions=1 diffusion.repetitions_test=1 model.name=basic_unet metrics=metrics_handler_binary_guidance loss=x_0_pred_loss_binary dataset.data_path=data/MNIST_more_digits
