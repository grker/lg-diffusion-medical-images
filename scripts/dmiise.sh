...
# epsilon
python src/main.py project_name=dmiise diffusion=base_diffusion diffusion.prediction_type=sample diffusion.noise_steps=100 diffusion.num_inference_steps=100 validation_period=30 trainer.max_epochs=500 dataloader.batch_size=64 dataloader.val_batch_size=256 dataset=mnist_labeled model.name=basic_unet metrics=metrics_handler_binary_guidance loss=mse_loss dataset.data_path=data/MNIST_modified optimizer=optimizer_ema optimizer.lr=0.001 optimizer.weight_decay=0.000001 seed=3874 dataset.mask_transformer.train_switch=False dataset.mask_transformer.threshold=0.0

# python src/main.py project_name=dmiise diffusion=base_diffusion diffusion.prediction_type=epsilon diffusion.noise_steps=100 diffusion.num_inference_steps=100 validation_period=20 trainer.max_epochs=300 dataloader.batch_size=16 dataloader.val_batch_size=64 dataset=bccd model.name=basic_unet model.emb_channels=128 metrics=metrics_handler_binary_bccd loss=mse_loss dataset.data_path=data/BCCD optimizer=optimizer_ema optimizer.lr=0.001 optimizer.weight_decay=0.00000001 seed=345 dataset.mask_transformer.train_switch=False dataset.mask_transformer.threshold=0.0


# python src/main.py project_name=dmiise diffusion=sample_diffusion diffusion.noise_steps=100 validation_period=20 trainer.max_epochs=400 dataloader.batch_size=64 dataloader.val_batch_size=256 dataset=acdc diffusion.repetitions=1 diffusion.repetitions_test=1 model.name=basic_unet metrics=metrics_handler_multi loss=x_0_pred_loss dataset.data_path=data/ACDC optimizer=optimizer_ema optimizer.lr=0.001 optimizer.weight_decay=0.0000001 seed=3874 diffusion.p=0.7
